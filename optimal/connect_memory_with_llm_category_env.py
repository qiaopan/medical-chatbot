...
def retrieve_documents(db, query: str) -> List[Document]:
...
def format_source_for_print(doc: Document, index: int, directory_labels: Dict[str, str]) -> str:
    return f"[{index}] {make_citation_path(doc)}"


def answer_with_rag(user_query: str):
    llm = get_llm(temperature=0.0, max_tokens=900)
    db = load_vectorstore()

    redacted_query, classification = classify_and_anonymize_user_query(user_query)
    ...
    docs = retrieve_documents(db, rewritten_query)
...
"""
    ...
    return answer, redacted_query, enhanced_query, rewritten_query, classification, docs


def print_classification_labels(classification: Dict[str, Any]) -> None:
    fields_to_print = [
        "main_category",
        "secondary_category",
        "vaccine_or_disease",
        "medicine_or_treatment",
        "target_population",
        "caller_type",
        "clinical_scenario",
        "age_or_life_stage",
    ...
    for field in fields_to_print:
        print(f"{field}: {classification.get(field)}")


def main():
    user_query = input("Write Query Here: ")
    answer, redacted_query, enhanced_query, rewritten_query, classification, docs = answer_with_rag(user_query)
    ...
    directory_labels = build_directory_labels(docs)
    for i, doc in enumerate(docs, start=1):
        print(format_source_for_print(doc, i, directory_labels))

    print("\nCLASSIFICATION LABELS:")
    print_classification_labels(classification)
